...
CREATE TABLE survey_surveyuser as SELECT DISTINCT global_id FROM (select global_id from epidb_results_intake union select global_id from epidb_results_weekly) t where global_id is not null and global_id != '';
